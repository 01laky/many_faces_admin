...
function caps(partial: Partial<MeCapabilities>): MeCapabilities {
	return {
		...
		requestFaceIndex: partial.requestFaceIndex ?? 'admin',
		isAdminFaceScope: partial.isAdminFaceScope ?? true,
...
describe('parseMeCapabilities', () => {
	...
	it('accepts duplicate permission strings in payload', () => {
		const parsed = parseMeCapabilities({
			globalRole: 'USER',
			requestFaceId: 1,
			requestFaceIndex: 'admin',
	...
	it('treats unknown permission strings as opaque', () => {
