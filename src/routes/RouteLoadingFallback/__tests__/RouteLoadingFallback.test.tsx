...
/**
 * GPL-22 (global app preloader, admin): the lazy-route `Suspense` fallback must render the
 * small inline preloader, never the bootstrap shell. Admin code-splits every page through
 * `lazyAdminPages`, so repainting the fixed full-viewport brand animation on each first
 * visit to a section would make an ordinary navigation look like a cold start.
 *
 * Pipeline position: `AppRoutes` wraps the whole `<Routes>` tree in
