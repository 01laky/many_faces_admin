...
export function renderBootstrapError(root: HTMLElement, apiUrl: string, onRetry: () => void): void {
	root.innerHTML =
		...
		'<p style="margin:0 0 0.5rem;color:#64748b;font-size:0.875rem;max-width:28rem">Check that the API is running at <code style="background:#f1f5f9;padding:0.15rem 0.35rem">' +
		escapeHtml(apiUrl) +
		'</code>, then retry.</p>' +
		'<p style="color:#94a3b8;font-size:0.8rem">Endpoint: <code>GET /api/localization/admin</code></p>' +
